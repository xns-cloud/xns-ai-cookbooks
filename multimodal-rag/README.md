...
- **Single-process, in-memory index.** No concurrency, no restart recovery,
  ...
  vector store.
- **Happy path only.** Audio is extracted, compressed, and split into
  10-minute segments before transcription — the transcription API caps
  uploads at 25 MB and the gpt-4o-transcribe models cap audio context
  around 25 minutes, both discovered the hard way. There is still no
  handling for unsupported codecs, API rate limits, or partial failures,
  and whole objects are held in memory during processing — a 3 GB video
  needs 3 GB of RAM.
